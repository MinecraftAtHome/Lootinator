...
std::size_t estimate_union_size(const SparseFactor& a, const SparseFactor& b) {
    ...
    std::size_t size = 1;
    for (const auto& [_, extent] : shape_by_pool) {
...
std::map<int, std::vector<int>> build_item_to_pools(const std::vector<NormalizedLootPool>& pools,
    ...
    for (const auto& [item_id, pool_set] : item_to_pool_set) {
        result[item_id] = std::vector<int>(pool_set.begin(), pool_set.end());
    }
    return result;
}
...
std::map<PoolItemKey, PoolItemStats> aggregate_pool_item_stats(const std::vector<NormalizedLootPool>& pools) {
    ...
    for (const auto& pool : pools) {
        ...
        for (const auto& [item_id, q_ld] : hit_probability_by_item) {
            const double q = static_cast<double>(q_ld);
            const auto& mass = count_mass_by_item[item_id];

            PoolItemStats item_stats;
            item_stats.q = q;
            item_stats.counts.reserve(mass.size());
            item_stats.conditional_pmf.reserve(mass.size());
            for (const auto& [count, probability] : mass) {
                item_stats.counts.push_back(count);
                item_stats.conditional_pmf.push_back(static_cast<double>(probability / q_ld));
            }
            item_stats.max_count = item_stats.counts.empty() ? 0 : item_stats.counts.back();
            stats[{pool.pool_index, item_id}] = std::move(item_stats);
        }
    }
...
std::map<int, int> compute_max_reachable_count_by_item(
    ...
    std::map<int, int> out;
    for (const auto& [item_id, pool_scope] : item_to_pools) {
        int total = 0;
        for (int pool_index : pool_scope) {
            total += rolls_max_by_pool.at(pool_index) * pool_item_stats.at({pool_index, item_id}).max_count;
        }
        out[item_id] = total;
    }
    return out;
...
std::map<int, int> compute_max_allowed_count_by_item(const std::map<int, std::vector<int>>& item_to_allowed_counts) {
    std::map<int, int> out;
    for (const auto& [item_id, allowed] : item_to_allowed_counts) {
        out[item_id] = allowed.empty() ? -1 : allowed.back();
    }
    return out;
}
...
std::map<PoolItemKey, PoolItemSubtotalDP> precompute_subtotal_dp_cache(const PreparedLootModel& prepared) {
    ...
    for (const auto& [item_id, allowed_counts] : prepared.item_to_allowed_counts) {
        const auto& pool_scope = prepared.item_to_pools.at(item_id);
        if (allowed_counts.empty() || pool_scope.empty()) continue;

        const int max_total = std::min(prepared.max_allowed_count_by_item.at(item_id),
                                       prepared.max_reachable_count_by_item.at(item_id));
        for (int pool_index : pool_scope) {
            cache[{pool_index, item_id}] = build_pool_item_subtotal_dp(
                prepared.pool_item_stats.at({pool_index, item_id}),
...
std::pair<std::vector<int>, std::vector<double>> build_constrained_item_probability_tensor(
    ...
    for (int pool_index : pool_scope) {
        ...
        auto [next_shape, next_state] = append_rows_truncated(state_shape, state, rows, max_total);
        state_shape = std::move(next_shape);
...
std::vector<SparseFactor> build_item_factors(
    ...
    for (const auto& [item_id, pool_scope] : prepared.item_to_pools) {
        const auto allowed_it = prepared.item_to_allowed_counts.find(item_id);
        ...
        if (allowed_it == prepared.item_to_allowed_counts.end()) {
            for (int pool_index : pool_scope) {
                const auto& stats = prepared.pool_item_stats.at({pool_index, item_id});
        ...
        auto [shape, coeff] = build_constrained_item_probability_tensor(prepared, item_id, subtotal_dp_cache);
        double log_scale = 0.0;

        for (std::size_t axis = 0; axis < pool_scope.size(); ++axis) {
            const int pool_index = pool_scope[axis];
            const auto& stats = prepared.pool_item_stats.at({pool_index, item_id});
            const auto unary = build_hit_weight_factor(pool_index, stats.q, prepared.log_factorials_by_pool.at(pool_index));
            multiply_tensor_by_axis_vector(coeff, shape, static_cast<int>(axis), unary.tensor);
            log_scale += unary.log_scale;
        }

        factors.push_back(normalize_factor(pool_scope, shape, coeff, log_scale));
    }
...
SparseFactor eliminate_pool_roll_count(const SparseFactor& factor,
    ...
    const auto& [weights, weights_log_scale] = prepared.scaled_roll_weights_by_pool.at(pool_index);
    if (factor.shape[axis] != static_cast<int>(weights.size())) {
    ...
    for (std::size_t i = 0; i < factor.tensor.size(); ++i) {
        ...
        const double weight = weights[coords[axis]];
        if (weight == 0.0) continue;
        coords.erase(coords.begin() + axis);
        reduced[reduced_layout.ravel(coords)] += value * weight;
    }

    return normalize_factor(new_scope, new_shape, reduced, factor.log_scale + weights_log_scale);
}
...
std::vector<int> choose_min_fill_elimination_order(const PreparedLootModel& prepared,
    ...
    for (const auto& [pool_index, _] : prepared.rolls_max_by_pool) pool_indices.push_back(pool_index);
...
double get_loot_probability(const LootTable& loot_table,
    ...
    for (const auto& [item_id, allowed_counts] : prepared.item_to_allowed_counts) {
        if (allowed_counts.empty()) return 0.0;
        if (prepared.max_reachable_count_by_item.at(item_id) < allowed_counts.front()) return 0.0;
    }

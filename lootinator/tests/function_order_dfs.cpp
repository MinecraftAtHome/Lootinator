...
[
	{
		...
		std::vector<data::LootFunctionType> target_order({
			data::LootFunctionType::APPLY_DAMAGE,
		...
		bool target_matches_result = std::equal(
			target_order.begin(), target_order.end(),
			conf.function_order.begin()
		);

...
namespace kgen {
    void SecondaryBruteforceKernel::gen_kernels(data::LootTableRoot& root_node, std::vector<ConfiguredKernel>& out, kgen::KernelGenConfig kgen_config) {
        SecondaryBruteforceKernel bk(root_node, kgen_config);
        out.push_back(bk.generate());
    }

    SecondaryBruteforceKernel::SecondaryBruteforceKernel(data::LootTableRoot &root_node, kgen::KernelGenConfig kgen_config) : Kernel(root_node, kgen_config) {}
...
R"(
    ...
    void SecondaryBruteforceKernel::emit_cuda_for_pool(std::ostream &out, data::LootPool *pool, int pool_idx) {

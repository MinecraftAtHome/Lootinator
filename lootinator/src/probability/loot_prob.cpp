...
namespace prob {
	...
	struct SparseFactor {
		...
		bool is_scalar() const { return scope.empty(); }

		std::size_t size() const { return tensor.size(); }
	};
	...
	std::size_t estimate_union_size(const SparseFactor& a, const SparseFactor& b) {
		...
		for (std::size_t i = 0; i < a.scope.size(); ++i)
			shape_by_pool[a.scope[i]] = a.shape[i];
		for (std::size_t i = 0; i < b.scope.size(); ++i)
			shape_by_pool.emplace(b.scope[i], b.shape[i]);
	...
	SparseFactor normalize_factor(const std::vector<int>& scope, const std::vector<int>& shape,
		const std::vector<double>& tensor, double log_scale = 0.0) {
	...
	SparseFactor scalar_factor(double value) {
		return value > 0.0 ? normalize_factor({}, {}, {value})
						   : SparseFactor{{}, {}, {0.0}, NEG_INF};
	}
	...
	double factor_value(const SparseFactor& factor) {
		if (factor.is_zero())
			return 0.0;
		if (!factor.is_scalar())
			throw std::runtime_error("Factor is not scalar.");
		if (factor.tensor[0] <= 0.0)
			return 0.0;
	...
	void validate_loot_table_light(const LootTable& loot_table) {
		...
		for (const auto& pool : loot_table.pools) {
			...
			for (const auto& entry : pool.entries) {
				if (entry.count_min < 0 || entry.count_max < entry.count_min ||
					entry.weight < 0.0) {
	...
	std::vector<TargetItem> validate_target_items_light(
		const std::vector<TargetItem>& target_items, const LootTable& loot_table) {
	...
	std::vector<NormalizedLootPool> expand_loot_table(const LootTable& loot_table) {
		...
		for (int pool_index = 0; pool_index < static_cast<int>(loot_table.pools.size());
			 ++pool_index) {
			...
			normalized.push_back(NormalizedLootPool{
				pool_index, pool.rolls_min, pool.rolls_max, std::move(outcomes)});
		}
		return normalized;
	}

	std::map<int, std::vector<int>> build_item_to_pools(
		const std::vector<NormalizedLootPool>& pools, const std::set<int>& all_item_ids) {
		std::map<int, std::set<int>> item_to_pool_set;
		for (int item_id : all_item_ids)
			item_to_pool_set[item_id] = {};
	...
	std::map<PoolItemKey, PoolItemStats> aggregate_pool_item_stats(
		...
		for (const auto& pool : pools) {
			...
			for (const auto& outcome : pool.outcomes) {
				hit_probability_by_item[outcome.item_id] +=
					static_cast<long double>(outcome.probability);
				count_mass_by_item[outcome.item_id][outcome.item_count] +=
					static_cast<long double>(outcome.probability);
			}
			...
			for (const auto& pair : hit_probability_by_item) {
				...
				for (const auto& pair2 : mass) {
					...
					item_stats.conditional_pmf.push_back(
						static_cast<double>(pair2.second / pair.second));
				}
	...
	std::map<int, std::vector<double>> precompute_log_factorials_by_pool(
		const std::vector<NormalizedLootPool>& pools) {
	...
	std::map<int, std::pair<std::vector<double>, double>> precompute_scaled_roll_weights_by_pool(
		...
		for (const auto& pool : pools) {
			...
			const double log_scale =
				-std::log(static_cast<double>(width)) + log_factorials[pool.rolls_max];
	...
	std::map<int, int> compute_max_reachable_count_by_item(
		...
		for (const auto& pair : item_to_pools) {
			...
			for (int pool_index : pair.second) {
				total += rolls_max_by_pool.at(pool_index) *
						 pool_item_stats.at({pool_index, pair.first}).max_count;
			}
	...
	std::map<int, std::vector<int>> convert_target_items_to_allowed_counts(
		...
		for (const auto& target : target_items) {
			...
			if (target.need_exact_count) {
				if (target.item_count <= max_reachable)
					allowed.push_back(target.item_count);
	...
	std::map<int, int> compute_max_allowed_count_by_item(
		const std::map<int, std::vector<int>>& item_to_allowed_counts) {
		std::map<int, int> out;
	...
	PreparedLootModel prepare_loot_model(
		const LootTable& loot_table, const std::vector<TargetItem>& target_items) {
		...
		model.scaled_roll_weights_by_pool =
			precompute_scaled_roll_weights_by_pool(model.pools, model.log_factorials_by_pool);
		for (const auto& pool : model.pools)
			model.rolls_max_by_pool[pool.pool_index] = pool.rolls_max;
		...
		model.max_allowed_count_by_item =
			compute_max_allowed_count_by_item(model.item_to_allowed_counts);
		return model;
	}

	PoolItemSubtotalDP build_pool_item_subtotal_dp(
		const PoolItemStats& stats, int max_rolls, int max_total) {
		...
		for (int hits = 1; hits <= max_rolls; ++hits) {
			...
			for (std::size_t i = 0; i < stats.counts.size(); ++i) {
				...
				if (probability == 0.0 || item_count > max_total)
					continue;
			...
			subtotal.swap(next);
			std::copy(subtotal.begin(),
				subtotal.end(),
				dp.subtotal_pmf.begin() + static_cast<std::size_t>(hits) * dp.cols);
		}

		return dp;
	}

	std::map<PoolItemKey, PoolItemSubtotalDP> precompute_subtotal_dp_cache(
		...
		for (const auto& pair : prepared.item_to_allowed_counts) {
			...
			if (pair.second.empty() || pool_scope.empty())
				continue;
	...
	SparseFactor build_hit_weight_factor(
		int pool_index, double q, const std::vector<double>& log_factorials) {
	...
	std::pair<std::vector<int>, std::vector<double>> append_rows_truncated(
		const std::vector<int>& state_shape, const std::vector<double>& state,
		const PoolItemSubtotalDP& rows, int max_total) {
		const int subtotal_len = max_total + 1;
		if (state_shape.empty() || state_shape.back() != subtotal_len ||
			rows.cols != subtotal_len) {
			throw std::runtime_error("Invalid shapes for subtotal append.");
		}

		const int prefix_cells =
			product(std::vector<int>(state_shape.begin(), state_shape.end() - 1));
			...
			for (int old_total = 0; old_total <= max_total; ++old_total) {
				...
				for (int r = 0; r < rows.rows; ++r) {
					const std::size_t row_offset =
						(static_cast<std::size_t>(prefix) * rows.rows + r) * subtotal_len;
	...
	std::pair<std::vector<int>, std::vector<double>> build_constrained_item_probability_tensor(
		const PreparedLootModel& prepared, int item_id,
		...
		for (int count : allowed_counts) {
			if (0 <= count && count <= reachable_max)
				allowed_indices.push_back(count);
		}
	...
	void multiply_tensor_by_axis_vector(std::vector<double>& tensor, const std::vector<int>& shape,
		int axis, const std::vector<double>& vec) {
		if (shape.empty()) {
			tensor[0] *= vec[0];
			return;
		}
	...
	std::vector<SparseFactor> build_item_factors(const PreparedLootModel& prepared,
		const std::map<PoolItemKey, PoolItemSubtotalDP>& subtotal_dp_cache) {
		...
		for (const auto& pair : prepared.item_to_pools) {
			...
			if (pair.second.empty()) {
				if (allowed_it != prepared.item_to_allowed_counts.end()) {
					const bool ok =
						std::find(allowed_it->second.begin(), allowed_it->second.end(), 0) !=
			...
			if (allowed_it == prepared.item_to_allowed_counts.end()) {
				for (int pool_index : pair.second) {
					...
					factors.push_back(build_hit_weight_factor(
						pool_index, stats.q, prepared.log_factorials_by_pool.at(pool_index)));
				}
				continue;
			}
			...
			const bool any_reachable = std::any_of(allowed_counts.begin(),
				allowed_counts.end(),
				[reachable_max](int c) { return 0 <= c && c <= reachable_max; });
			if (!any_reachable) {
			...
			// pair2 = [shape, coeff]
			auto pair2 =
				build_constrained_item_probability_tensor(prepared, pair.first, subtotal_dp_cache);
			double log_scale = 0.0;

			for (std::size_t axis = 0; axis < pair.second.size(); ++axis) {
				const int pool_index = pair.second[axis];
				const auto& stats = prepared.pool_item_stats.at({pool_index, pair.first});
				const auto unary = build_hit_weight_factor(
					pool_index, stats.q, prepared.log_factorials_by_pool.at(pool_index));
				multiply_tensor_by_axis_vector(
					pair2.second, pair2.first, static_cast<int>(axis), unary.tensor);
	...
	SparseFactor multiply_two_factors(
		const SparseFactor& a, const SparseFactor& b, const PreparedLootModel& prepared) {
		if (a.is_zero() || b.is_zero())
			return scalar_factor(0.0);
		...
		for (std::size_t i = 0; i < a.scope.size(); ++i)
			a_axis[a.scope[i]] = static_cast<int>(i);
		for (std::size_t i = 0; i < b.scope.size(); ++i)
			b_axis[b.scope[i]] = static_cast<int>(i);
	...
	SparseFactor multiply_many_factors(
		std::vector<SparseFactor> factors, const PreparedLootModel& prepared) {
		std::vector<SparseFactor> active;
		active.reserve(factors.size());
		for (const auto& factor : factors) {
			if (factor.is_scalar() && factor_value(factor) == 1.0)
				continue;
		...
		while (active.size() > 1) {
			...
			std::tuple<int, std::size_t, std::size_t> best_score{std::numeric_limits<int>::max(),
			...
			for (int i = 0; i < static_cast<int>(active.size()); ++i) {
				for (int j = i + 1; j < static_cast<int>(active.size()); ++j) {
					...
					const auto score = std::make_tuple(static_cast<int>(scope.size()),
			...
			SparseFactor merged =
				multiply_two_factors(active[best_pair.first], active[best_pair.second], prepared);
	...
	SparseFactor eliminate_pool_roll_count(
		const SparseFactor& factor, int pool_index, const PreparedLootModel& prepared) {
		if (factor.is_zero())
			return factor;

		const auto it = std::find(factor.scope.begin(), factor.scope.end(), pool_index);
		if (it == factor.scope.end())
			return factor;
	...
	std::vector<int> choose_min_fill_elimination_order(
		const PreparedLootModel& prepared, const std::vector<SparseFactor>& factors) {
		std::vector<int> pool_indices;
		// pair = [pool_index, _]
		for (const auto& pair : prepared.rolls_max_by_pool)
			pool_indices.push_back(pair.first);
		std::sort(pool_indices.begin(), pool_indices.end());

		std::map<int, std::set<int>> graph;
		for (int pool_index : pool_indices)
			graph[pool_index] = {};
		for (const auto& factor : factors) {
			...
			for (int pool_index : remaining) {
				...
				for (std::size_t i = 0; i < neighbors.size(); ++i) {
					for (std::size_t j = i + 1; j < neighbors.size(); ++j) {
						if (!graph[neighbors[i]].count(neighbors[j]))
							++fill;
					}
				}

				const auto score = std::make_tuple(fill,
	...
	double get_loot_probability(
		...
		for (const auto& pair : prepared.item_to_allowed_counts) {
			if (pair.second.empty())
				return 0.0;
			if (prepared.max_reachable_count_by_item.at(pair.first) < pair.second.front())
				return 0.0;
		}
		...
		for (const auto& factor : active_factors) {
			if (factor.is_zero() && factor.is_scalar())
				return 0.0;
		}
		...
		for (int pool_index : choose_min_fill_elimination_order(prepared, active_factors)) {
			...
			for (const auto& factor : active_factors) {
				if (std::find(factor.scope.begin(), factor.scope.end(), pool_index) !=
					factor.scope.end()) {
			...
			if (!involved.empty()) {
				SparseFactor reduced = eliminate_pool_roll_count(
					multiply_many_factors(involved, prepared), pool_index, prepared);
				untouched.push_back(std::move(reduced));
				if (untouched.back().is_zero() && untouched.back().is_scalar())
					return 0.0;
			}
	...
	LootTable get_probability_table(const data::LootTableRoot& tbl_in) {
		...
		for (auto child : tbl_in.children) {
			CAST_CHILD(pool_in, data::LootPool, child);
			LootPool pool_out;
			pool_out.rolls_min = pool_in->rolls.min;
			pool_out.rolls_max = pool_in->rolls.max;
			for (auto child2 : pool_in->children) {
				CAST_CHILD(entry_in, data::LootEntry, child2);
				util::RangeInclusive<uint32_t> count_range = entry_in->get_count_range();
				LootEntry entry_out = {
					entry_in->item,
			...
			tbl_out.pools.push_back(pool_out);
		}

		return tbl_out;
	}

	static std::vector<TargetItem> get_target_items_from_constraints(
	...
	double get_probability_of_config(kgen::KernelGenConfig& kgen_config) {
		data::LootTableRoot root = data::LootTableRoot(
			kgen_config.loot_table_json, kgen_config.item_map, kgen_config.version);
		prob::LootTable loot_table = prob::get_probability_table(root);
		std::vector<TargetItem> target_items =
			get_target_items_from_constraints(kgen_config.constraints);
		return get_loot_probability(loot_table, target_items);
	}
	...
	//                 2,
	//                 {
	//                     LootEntry{1, 0.50, 1, 1},
	//                     LootEntry{2, 0.30, 1, 2},
	//                     LootEntry{1, 0.20, 2, 3},
	//                 }
	//             },
	//             LootPool{
	//                 0,
	...
	//
	//     std::vector<TargetItem> target_items{
	//         TargetItem{1, 2, false}, // at least 2 of item 1002
	//     };
	//

...
namespace launcher {
	...
	constexpr u32 BENCHMARK_BATCH_SCALE =
		4u; // real batch size is this times bigger than test batch size
	constexpr float BENCHMARK_MAX_ELAPSED_TIME =
		100.0f; // elapsed milliseconds of single run that stop the benchmark

#define CUDA_CHECK(ans)                                                                            \
	do {                                                                                           \
		launcher::gpuAssert((ans), __FILE__, __LINE__);                                            \
	} while (false)
...
#define NVRTC_CHECK(ans)                                                                           \
	do {                                                                                           \
		launcher::nvrtcAssert((ans), __FILE__, __LINE__);                                          \
	} while (false)
	void nvrtcAssert(nvrtcResult res, const char* file, int line) {
		if (res != NVRTC_SUCCESS) {
			std::cerr << "NVRTC error: " << nvrtcGetErrorString(res) << " at " << file << ":"
					  << line << std::endl;
			exit(1);
		}
	}

#define DEBUG(bool_var)                                                                            \
	if (bool_var)                                                                                  \
	std::cerr << "Debug: "
	...
	struct KernelData {
		...
		KernelData(const LaunchParameters& config) {
			...
			CUDA_CHECK(cuMemAlloc(
				&d_shared_mem_contents, config.kernel_shared_memory.size() * sizeof(u32)));
			...
			CUDA_CHECK(cuMemcpyHtoD(d_shared_mem_contents,
				config.kernel_shared_memory.data(),
				h_shared_mem_contents_length * sizeof(u32)));

			// __global__ kernel_name(u64* result_array, u32* result_count, u32*
			// shared_mem_contents, u32 shared_mem_contents_length, u64 offset)
...
namespace launcher {
	int finalize_config(LaunchParameters& config, const AppParameters& app_params) {
		...
		const launcher::u32 n_blocks =
			static_cast<launcher::u32>(config.threads_per_batch / config.threads_per_block);
		...
		if (static_cast<launcher::u64>(n_blocks) != n_blocks_long) {
			std::cerr << "Fatal error (launch_kernel): number of blocks exceeded 2^32 for provided "
						 "thread block size: "
		...
		if (config.start_batch == launcher::UNSPECIFIED || config.start_batch < 0) {
			DEBUG(app_params.debug_info)
				<< "Start batch unspecified or too small, defaulting to start-batch=0" << std::endl;
			config.start_batch = 0;
		}
		launcher::i32 end_exclusive =
			(config.threads_total + config.threads_per_batch - 1) / config.threads_per_batch;
		if (config.end_batch == launcher::UNSPECIFIED || config.end_batch > end_exclusive) {
			DEBUG(app_params.debug_info)
				<< "End batch unspecified or too large, defaulting to end-batch=" << end_exclusive
				<< std::endl;
	...
	// Uses cuda driver api to turn the provided parameters into a functional kernel and launches
	// it.
	int launch_kernel(
		KernelData& kdata, const LaunchParameters& config, const AppParameters& app_params) {
		u64 h_result_array[RESULT_BUFFER_SIZE];
		u32 h_shared_mem_contents_length = config.kernel_shared_memory.size();

		for (i32 batch = config.start_batch; batch < config.end_batch; batch++) {
			// DEBUG(app_params.debug_info) << "Running batch #" << batch << " of range [" <<
			// config.start_batch << ", " << config.end_batch << ")" << std::endl;
			...
			CUDA_CHECK(cuLaunchKernel(kdata.kernel,
				n_blocks,
				1,
				1, // grid size
			...
			CUDA_CHECK(
				cuMemcpyDtoH(h_result_array, kdata.d_result_array, h_result_count * sizeof(u64)));
		...
		DEBUG(app_params.debug_info) << "launch_kernel finished. work done: " << config.start_batch
									 << " " << config.end_batch << "\n";
		return 0;
	}

	// Performs an automated benchmark of the provided kernel.
	BenchmarkResults benchmark_kernel(
		const LaunchParameters& config, const AppParameters& app_params) {
		...
		DEBUG(app_params.debug_info)
			<< " benchmarking kernel " << config.kernel_name << " - running benchmark";
	...
	void write_human_readable_time(std::ofstream& fout, const float time_ms) {
		...
		fout << full_hours << " hours, " << full_minutes << " minutes, " << full_seconds
			 << " seconds";
	}

	// Benchmarks all kernels provided in the vector of launch parameters. Prints results to
	// BENCHMARK_RESULTS_FILE.
	void benchmark_all(
		const AppParameters& app_params, std::vector<launcher::LaunchParameters> kernel_configs) {
		std::vector<BenchmarkResults> result_vec;

		for (const auto& config : kernel_configs) {
			DEBUG(app_params.debug_info)
				<< " benchmarking kernel " << config.kernel_name << " - initializing data\n";
			BenchmarkResults res = benchmark_kernel(config, app_params);
			DEBUG(app_params.debug_info)
				<< " benchmarking kernel " << config.kernel_name << " - done\n";

			result_vec.push_back(res);
			if (!res.success) {
				std::cerr << "Warning: Benchmark failed for kernel " << config.kernel_name
		...
		std::sort(
			result_vec.begin(), result_vec.end(), [](BenchmarkResults& l, BenchmarkResults& r) {
				return l.ms_total_estimate < r.ms_total_estimate;
			});
	...
	int compile_nvrtc(
		...
		if (file.fail() && !file.eof()) {
			std::cerr << "Fatal error (compile_nvrtc): Failed while reading source code file '"
					  << source_filename << "'." << std::endl;
			return 1;
		}
		...
		NVRTC_CHECK(
			nvrtcCreateProgram(&prog, kernel_source.c_str(), "internal.cu", 0, nullptr, nullptr));
	...
	int read_shared_memory(const char* shared_mem_filename, LaunchParameters& config) {
		...
		if (!file) {
			std::cerr << "Fatal error (read_shared_memory): Failed to open file '"
					  << shared_mem_filename << "'." << std::endl;
			return 1;
		}
	...
	int parse_config_json(const char* config_filename, AppParameters& app_params,
		std::vector<LaunchParameters>& launch_params) {
		try {
			...
			for (auto& node : kernel_data) {
				...
				if (node.contains("start_batch"))
					lp.start_batch = node["start_batch"];
				if (node.contains("start_batch"))
					lp.end_batch = node["end_batch"];

				DEBUG(app_params.debug_info) << "Compiling kernel " << lp.kernel_name << "...\n";

				if (read_shared_memory(shmem_file.c_str(), lp)) {
					std::cerr << "Error: failed to read shared memory file '" << shmem_file
	...
	int parse_args(int argc, char** argv, AppParameters& app_params,
		std::vector<LaunchParameters>& launch_params) {
		// default app config
		...
		for (int i = 1; i < argc; i++) {
			...
			if (strcmp(argv[i], "--benchmark") == 0) {
				if (app_params.mode == AppMode::RUN_SINGLE) {
					std::cerr << "Error: Illegal args: --benchmark and --run-single cannot be "
								 "specified at once!\n";
					return 1;
				}
		...
		if (launch_params.empty()) {
			std::cerr << "Error: Unspecified configuration file. Did you forget to --use-config "
						 "(filename.json)?\n";
			return 1;
		}

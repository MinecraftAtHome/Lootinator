...
namespace loot {
	KernelTemplate::KernelTemplate(const TemplateParameters& params, const std::string& kernel_name)
		: Template(params) {
...
__device__ inline int nextIntNoAdvance(uint64_t *rand, const int n) {if ((n-1 & n) == 0) {uint64_t x = n * *rand; return (int)((int64_t)x >> 31);} else {return (int)(*rand % n);}} 
	...
	// Generates a kernel header - copies necessary data to __shared__ memory and creates unique
	// thread index tid.
